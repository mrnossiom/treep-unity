...
namespace Mirror {
    ...
    public static partial class NetworkServer {
        ...
        static void Initialize() {
            ...
            Debug.Assert(Transport.active != null,
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        internal static void RegisterMessageHandlers() {
            ...
            RegisterHandler<TimeSnapshotMessage>(OnTimeSnapshotMessage,
                false); // unreliable may arrive before reliable authority went through
        }
        ...
        static void OnCommandMessage(NetworkConnectionToClient conn, CommandMessage msg, int channelId) {
            if (!conn.isReady) {
                ...
                if (channelId == Channels.Reliable) {
                    ...
                    if (spawned.TryGetValue(msg.netId, out NetworkIdentity netIdentity))
                        if (msg.componentIndex < netIdentity.NetworkBehaviours.Length &&
                            netIdentity.NetworkBehaviours[msg.componentIndex] is NetworkBehaviour component)
                            if (RemoteProcedureCalls.GetFunctionMethodName(msg.functionHash, out string methodName)) {
                                Debug.LogWarning(
                                    $"Command {methodName} received for {netIdentity.name} [netId={msg.netId}] component {component.name} [index={msg.componentIndex}] when client not ready.\nThis may be ignored if client intentionally set NotReady.");
                                return;
                            }
            ...
            if (requiresAuthority && identity.connectionToClient != conn) {
                ...
                if (msg.componentIndex < identity.NetworkBehaviours.Length &&
                    identity.NetworkBehaviours[msg.componentIndex] is NetworkBehaviour component)
                    if (RemoteProcedureCalls.GetFunctionMethodName(msg.functionHash, out string methodName)) {
                        Debug.LogWarning(
                            $"Command {methodName} received for {identity.name} [netId={msg.netId}] component {component.name} [index={msg.componentIndex}] without authority");
                        return;
                    }
            ...
            using (NetworkReaderPooled networkReader = NetworkReaderPool.Get(msg.payload))
                identity.HandleRemoteCall(msg.componentIndex, msg.functionHash, RemoteCallType.Command, networkReader,
                    conn);
        }
                    ...
                    using (NetworkReaderPooled reader = NetworkReaderPool.Get(message.payload)) {
                        ...
                        if (!identity.DeserializeServer(reader)) {
                            if (exceptionsDisconnect) {
                                ...
                                Debug.LogWarning(
                                    $"Server failed to deserialize client state for {identity.name} with netId={identity.netId}.");
                        }
                    }
                }
        ...
        public static void SendToAll<T>(T message, int channelId = Channels.Reliable, bool sendToReadyOnly = false)
            ...
            using (NetworkWriterPooled writer = NetworkWriterPool.Get()) {
                ...
                if (writer.Position > max) {
                    Debug.LogError(
                        $"NetworkServer.SendToAll: message of type {typeof(T)} with a size of {writer.Position} bytes is larger than the max allowed message size in one batch: {max}.\nThe message was dropped, please make it smaller.");
                    return;
                }
        ...
        static void SendToObservers<T>(NetworkIdentity identity, T message, int channelId = Channels.Reliable)
            ...
            using (NetworkWriterPooled writer = NetworkWriterPool.Get()) {
                ...
                if (writer.Position > max) {
                    Debug.LogError(
                        $"NetworkServer.SendToObservers: message of type {typeof(T)} with a size of {writer.Position} bytes is larger than the max allowed message size in one batch: {max}.\nThe message was dropped, please make it smaller.");
                    return;
                }
        ...
        public static void SendToReadyObservers<T>(NetworkIdentity identity, T message, bool includeOwner = true,
            ...
            using (NetworkWriterPooled writer = NetworkWriterPool.Get()) {
                ...
                if (writer.Position > max) {
                    Debug.LogError(
                        $"NetworkServer.SendToReadyObservers: message of type {typeof(T)} with a size of {writer.Position} bytes is larger than the max allowed message size in one batch: {max}.\nThe message was dropped, please make it smaller.");
                    return;
                }
        ...
        internal static void OnTransportData(int connectionId, ArraySegment<byte> data, int channelId) {
            if (connections.TryGetValue(connectionId, out NetworkConnectionToClient connection)) {
                ...
                if (!connection.unbatcher.AddBatch(data)) {
                    if (exceptionsDisconnect) {
                        Debug.LogError(
                            $"NetworkServer: received message from connectionId:{connectionId} was too short (messages should start with message id). Disconnecting.");
                        connection.Disconnect();
                    }
                    else
                        Debug.LogWarning(
                            $"NetworkServer: received message from connectionId:{connectionId} was too short (messages should start with message id).");
                ...
                while (!isLoadingScene &&
                       connection.unbatcher.GetNextMessage(out ArraySegment<byte> message,
                        ...
                        if (reader.Remaining >= NetworkMessages.IdSize) {
                            ...
                            if (!UnpackAndInvoke(connection, reader, channelId)) {
                                ...
                                if (exceptionsDisconnect) {
                                    ...
                                    Debug.LogWarning(
                                        $"NetworkServer: failed to unpack and invoke message from connectionId:{connectionId}.");
        ...
        public static void RegisterHandler<T>(Action<NetworkConnectionToClient, T> handler,
        ...
        public static void RegisterHandler<T>(Action<NetworkConnectionToClient, T, int> handler,
            bool requireAuthentication = true)
            where T : struct, NetworkMessage {
            ushort msgType = NetworkMessageId<T>.Id;
            if (handlers.ContainsKey(msgType)) {
        ...
        /// <summary>Replace a handler for message type T. Most should require authentication.</summary>
        public static void ReplaceHandler<T>(Action<NetworkConnectionToClient, T> handler,
            bool requireAuthentication = true)
            where T : struct, NetworkMessage {
            ushort msgType = NetworkMessageId<T>.Id;
        ...
        /// <summary>Replace a handler for message type T. Most should require authentication.</summary>
        public static void ReplaceHandler<T>(Action<NetworkConnectionToClient, T, int> handler,
            bool requireAuthentication = true)
            where T : struct, NetworkMessage {
            ushort msgType = NetworkMessageId<T>.Id;
        ...
        public static bool AddPlayerForConnection(NetworkConnectionToClient conn, GameObject player) {
            if (!player.TryGetComponent(out NetworkIdentity identity)) {
                Debug.LogWarning(
                    $"AddPlayer: player GameObject has no NetworkIdentity. Please add a NetworkIdentity to {player}");
                return false;
            }
        ...
        [Obsolete(
            "Use ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, uint assetId, ReplacePlayerOptions replacePlayerOptions) instead")]
        public static bool ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, uint assetId,
            bool keepAuthority = false) {
            if (GetNetworkIdentity(player, out NetworkIdentity identity))
                identity.assetId = assetId;

            return ReplacePlayerForConnection(conn, player,
                keepAuthority ? ReplacePlayerOptions.KeepAuthority : ReplacePlayerOptions.KeepActive);
        }

        // Deprecated 2024-008-09
        [Obsolete(
            "Use ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, ReplacePlayerOptions replacePlayerOptions) instead")]
        public static bool ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player,
            bool keepAuthority = false) {
            return ReplacePlayerForConnection(conn, player,
                keepAuthority ? ReplacePlayerOptions.KeepAuthority : ReplacePlayerOptions.KeepActive);
        }
        ...
        public static bool ReplacePlayerForConnection(NetworkConnectionToClient conn, GameObject player, uint assetId,
            ReplacePlayerOptions replacePlayerOptions) {
            if (GetNetworkIdentity(player, out NetworkIdentity identity))
                identity.assetId = assetId;
        ...
        [Obsolete(
            "Use RemovePlayerForConnection(NetworkConnectionToClient conn, RemovePlayerOptions removeOptions) instead")]
        ...
        public static void RemovePlayerForConnection(NetworkConnectionToClient conn,
            RemovePlayerOptions removeOptions = RemovePlayerOptions.KeepActive) {
        ...
        static ArraySegment<byte> CreateSpawnMessagePayload(bool isOwner, NetworkIdentity identity,
            NetworkWriterPooled ownerWriter, NetworkWriterPooled observersWriter) {
        ...
        public static void Destroy(GameObject obj) {
            ...
            if (!active) {
                Debug.LogWarning(
                    "NetworkServer.Destroy() called without an active server. Servers can only destroy while active, clients can only ask the server to destroy (for example, with a [Command]), after which the server may decide to destroy the object and broadcast the change to all clients.");
                return;
            }
            ...
            if (!GetNetworkIdentity(obj, out NetworkIdentity identity)) {
                Debug.LogWarning(
                    $"NetworkServer.Destroy() called on {obj.name} which doesn't have a NetworkIdentity component.");
                return;
            }
